...
TitleScreenView::TitleScreenView(Model *model)
{
    this->model = dynamic_cast<TitleScreenModel*>(model);
    startGame = nullptr;
    options = nullptr;
    quit = nullptr;
}
...
{
    static SDL_Rect backgroundRect = {0, WINDOW_HEIGHT / 2, WINDOW_WIDTH, WINDOW_HEIGHT / 2};
    static SDL_Rect startGameRect = {(WINDOW_WIDTH - startGame->w) / 2 + 3, 5 * WINDOW_HEIGHT / 8 - startGame->h / 2 + 5, 0, 0};
    static SDL_Rect optionsRect = {(WINDOW_WIDTH - options->w) / 2 + 3, 3 * WINDOW_HEIGHT / 4 - options->h / 2 + 5, 0, 0};
    static SDL_Rect quitRect = {(WINDOW_WIDTH - quit->w) / 2 + 3, 7 * WINDOW_HEIGHT / 8 - quit->h / 2 + 5, 0, 0};
